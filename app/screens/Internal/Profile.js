	...
	async componentDidMount() {
		...
		this.setState({
			user: {
				...
				artists: [
					JSON.parse(artists).items[0].name,
					JSON.parse(artists).items[1].name,
					JSON.parse(artists).items[2].name,
				],
				tracks: [
					JSON.parse(tracks).items[0].name,
					JSON.parse(tracks).items[1].name,
					JSON.parse(tracks).items[2].name,
				],
			},
		});
		this.props.navigation.setOptions({
			title: this.state.user.info,
		});
	}
			...
			<View style={styles.container}>
				{this.state.user == null ? (
					...
					<View style={{ alignItems: 'center', marginTop: 100 }}>
						...
						<Text style={{ fontSize: 24, marginTop: 10, color: '#2AA7E7' }}>
						...
						<Text>Followers</Text> */}
							...
							>
								<View>
									<Text style={styles.topHeading}>Top Artists</Text>
								...
								<View>
									<Text style={styles.topHeading}>Top Tracks</Text>
									<View style={styles.topContainer}>
